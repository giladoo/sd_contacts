...
def stringify_leaf(leaf):
    ...
    stringify = "%s %s %s" % (
    right_operand if switcher else left_operand, operator, left_operand if switcher else right_operand)
    return stringify
...
autoreplace = input(
    'Do you want to auto-replace attributes ? (y/n) (empty == no) (will not ask confirmation for each file) : ') or 'n'
nofilesfound = True
ok_files = []
nok_files = []
        ...
        with open(xml_file, 'rb') as f:
            ...
            for state_tag in tags_with_states:
                ...
                invisible_attr = "state not in [%s]" % ','.join(
                    ("'" + state.strip() + "'") for state in state_tag['states'].split(','))
